...
bool UpdateExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) {
  ...
  if (exec_ctx_->GetLockManager()->IsLockShared(txn_, *rid)) {
    exec_ctx_->GetLockManager()->LockUpgrade(txn_, *rid);
    LOG_DEBUG("lock upgrade, transaction id is %d", txn_->GetTransactionId());
  }
  if (!exec_ctx_->GetLockManager()->IsLockExclusive(txn_, *rid)) {
    LOG_DEBUG("lock exclusive");
    exec_ctx_->GetLockManager()->LockExclusive(txn_, *rid);
  }
