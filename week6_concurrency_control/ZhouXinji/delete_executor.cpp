...
bool DeleteExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) {
  ...
  for (auto &index_info : index_infos) {
    ...
    exec_ctx_->GetTransaction()->GetIndexWriteSet().get()->emplace_back(IndexWriteRecord{
        *rid, table_info_->oid_, WType::DELETE, delete_tuple, index_info->index_oid_, exec_ctx_->GetCatalog()});
  }
  return deleted;
}
