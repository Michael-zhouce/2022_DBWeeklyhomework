...
#include "hash_table_directory_page.h"
#include <algorithm>
...
{
    int end = (1 << (global_depth_ + 1)) - 1;
    int interval = 1 << global_depth_;
    for (int start = 1 << global_depth_; start <= end; ++start)
    {
        bucket_page_ids_[start] = bucket_page_ids_[start - interval];
        local_depths_[start] = local_depths_[start - interval];
    }
    global_depth_++;
}
...
auto HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx)->uint32_t
{
    ...
    return bucket_idx ^ (1 << (local_depth - 1)); //按local_depth的最高位取反，这样可以得到兄弟bucket的id
}
...
{
    return 1<<global_depth_; //2的global_depth_次方，就是大小了
}

auto HashTableDirectoryPage::CanShrink() -> bool
{
    //判断是否所有的local_depth都比global_depth小
    uint32_t imax = Size();
