...
#include "hash_table_page_defs.h"
...
class HashTableDirectoryPage {
 public:
   ...
   * representation.
   * LSB是最低有效位的意思，该函数的意思是返回一个二进制数，然后1的个数是global_depth，其余高位全是0
 ...
 private:
	page_id_t page_id_;//目录也是页，这是目录自己的页编号
	lsn_t lsn_;//日志序列号，目前不知道干嘛的
	uint32_t global_depth_{0};//目录的全局深度
	uint8_t local_depths_[DIRECTORY_ARRAY_SIZE];//第i个桶的局部深度
    page_id_t bucket_page_ids_[DIRECTORY_ARRAY_SIZE];//第i个槽指向的bucket的id
	//故最后这俩数组的下标范围是：0到2^global_depth_-1
};
