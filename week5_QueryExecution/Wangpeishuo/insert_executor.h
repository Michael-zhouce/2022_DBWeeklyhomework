...
class InsertExecutor : public AbstractExecutor {
 ...
 private:
  ...
  const InsertPlanNode *plan_;//从plan里可以得到表格的id和要插入的值（可能为空，这是要找子节点）
  //从父类里还继承了ExecutorContext* exec_ctx_;
  //executorcontet里有transacion对象，catalog对象
  std::unique_ptr<AbstractExecutor> &child_executor_;
  //tableheap在tableinfo里，tableinfo在Catalog里！
  uint32_t idx; //若israwinsert为真，则代表当前plan_->raw_values_的下标
  Catalog* ctlg_; //目录
};
