...
func (rf *Raft) Election() {
	for !rf.killed() {
		...
		func() {
			...
			insts := now.Sub(rf.lastActicveTime)
			if rf.state == Follower {
				if insts > timeout {
					rf.state = Candidate
				}
			}
			if rf.state == Candidate && insts >= timeout {
				rf.lastActicveTime = time.Now()
				rf.currentTerm += 1
				rf.votedFor = rf.me
				...
				maxTerm := 0
				for true {
					if voteResult, ok := <-voteResultChan; ok == true {
						finishCount += 1
